...
get_weather <- function(cities,continent){
  ...
  for (i in 1:length(cities)){
    ...
    search_temp[i] <- content(GET(paste0(query_base,"location/",woeid[i],"/")))
    ws_name[i] <- search_temp[i][[1]][[1]]$weather_state_name
  ...
  loc_map <- ggmap(map)+
    ...
    geom_text(data = df, aes(x = lon, y = lat, label=paste(city_name," ",ws_name)))
  print(loc_map)
...
#Test examples
response <- get_weather(c("oslo","london"),"Europe")
